...
def _brand_slug(value):
    return "".join(ch for ch in str(value or "").lower() if ch.isalnum())


def sportsbook_logo_urls(book_key, book_name=""):
    """Return several logo candidates; the browser automatically tries the next one."""
    key = str(book_key or "").strip().lower()
    name = str(book_name or "").strip().lower()
    compact_key = _brand_slug(key)
    compact_name = _brand_slug(name)

    # Verified/common OpticOdds filenames plus aliases seen in odds feeds.
    aliases = {
        "draftkings": "draftkings", "fanduel": "fanduel", "betmgm": "betmgm",
        "caesars": "caesars", "bet365": "bet365", "fanatics": "fanatics",
        "espnbet": "espnbet", "betrivers": "betrivers", "hardrockbet": "hardrockbet",
        "hardrock": "hardrockbet", "circasports": "circasports", "circa": "circasports",
        "betonline": "betonline", "betonlineag": "betonline", "bovada": "bovada",
        "betus": "betus", "mybookie": "mybookie", "mybookieag": "mybookie",
        "fliff": "fliff", "betparx": "betparx", "betfred": "betfred",
        "goldennugget": "goldennugget", "betway": "betway", "pointsbet": "pointsbet",
        "unibet": "unibet", "williamhill": "williamhill", "lowvig": "lowvig",
        "lowvigag": "lowvig", "betanysports": "betanysports", "betanysportsag": "betanysports",
        "everygame": "everygame", "bookmaker": "bookmaker", "bookmakerag": "bookmaker",
        "superbook": "superbook", "betopenly": "betopenly", "betfair": "betfair",
        "pinnacle": "pinnacle", "betcris": "betcris", "betdsi": "betdsi",
        "bet105": "bet105", "bet105ag": "bet105", "heritage": "heritage",
        "heritagesports": "heritage", "sportsbettingag": "sportsbetting",
        "ballybet": "ballybet", "betjack": "betjack", "desertdiamond": "desertdiamond",
        "fourwinds": "fourwinds", "betly": "betly", "coral": "coral", "ladbrokes": "ladbrokes"
    }
    slug = aliases.get(compact_key) or aliases.get(compact_name) or compact_key or compact_name
    if not slug:
        return []

    base = "https://cdn.opticodds.com/sportsbook-logos/"
    # Different books on the CDN use different extensions, so don't assume one.
    candidates = [base + slug + ext for ext in (".jpg", ".jpeg", ".png")]

    # Reliable website-icon fallbacks for brands not currently present on the CDN.
    # These come after the normal full-size logo candidates, so existing logos are unchanged.
    fallback_domains = {
        ...
        "courtside": "courtside.app",
        "courtsideapp": "courtside.app",
    }
    fallback_domain = fallback_domains.get(compact_key) or fallback_domains.get(compact_name)
    if fallback_domain:
        candidates.append(
            "https://www.google.com/s2/favicons?domain=" + fallback_domain + "&sz=128"
        )

    # ESPN BET is known to be jpeg; put it first.
    if slug == "espnbet":
        candidates = [base + "espnbet.jpeg", base + "espnbet.jpg", base + "espnbet.png"]
    return candidates


_TEAM_LOGO_CACHE = {"timestamp": 0, "map": {}}

def _team_norm(value):
    text = str(value or "").lower().replace("&", "and")
    return " ".join("".join(ch if ch.isalnum() else " " for ch in text).split())

def get_college_logo_map():
    """Fetch ESPN's team directory server-side, avoiding browser CORS failures."""
    now = time.time()
    if _TEAM_LOGO_CACHE["map"] and now - _TEAM_LOGO_CACHE["timestamp"] < 21600:
        return _TEAM_LOGO_CACHE["map"]
    try:
        response = requests.get(
            "https://site.web.api.espn.com/apis/site/v2/"
    "sports/football/college-football/teams?limit=1000"
        )
        response.raise_for_status()
        entries = response.json().get("sports", [{}])[0].get("leagues", [{}])[0].get("teams", [])
        index = {}
        for entry in entries:
            ...
            if not logo:
                continue
            names = [team.get("displayName"), team.get("shortDisplayName"), team.get("name"),
                     team.get("location"), team.get("abbreviation"), team.get("slug")]
            for item in names:
                if item:
                    index[_team_norm(item)] = logo
        if index:
            _TEAM_LOGO_CACHE.update(timestamp=now, map=index)
        return index
    except Exception as error:
        print("TEAM LOGO FETCH ERROR:", repr(error))
        return _TEAM_LOGO_CACHE["map"]

def college_logo_url(team_name, logo_map):
    wanted = _team_norm(team_name)
    if wanted in logo_map:
        return logo_map[wanted]
    # Conservative containment fallback for feed names such as 'Colorado State Rams'.
    matches = [(name, url) for name, url in logo_map.items()
               if len(name) >= 4 and (wanted.startswith(name + " ") or name.startswith(wanted + " "))]
    if matches:
...
def parse_game(event):
    ...
    logo_map = get_college_logo_map()
    away_logo_url = college_logo_url(away_team, logo_map)
    home_logo_url = college_logo_url(home_team, logo_map)
...
<style>
    ...
    * {
    ...
    /* ======================================================
       ODDSCOPE PRO UX
       ====================================================== */
    .scroll-progress{position:fixed;top:0;left:0;height:2px;width:0;z-index:9999;background:linear-gradient(90deg,var(--green),var(--blue));box-shadow:0 0 16px rgba(52,211,153,.7);transition:width .08s linear}
    .signal-ticker{border:1px solid var(--border);background:rgba(13,19,29,.88);border-radius:14px;margin:16px 0 20px;overflow:hidden;display:flex;align-items:center;min-height:42px}
    .ticker-label{align-self:stretch;display:flex;align-items:center;padding:0 14px;background:var(--green-soft);color:var(--green);font-size:10px;font-weight:900;letter-spacing:.14em;white-space:nowrap;border-right:1px solid var(--border)}
    .ticker-window{overflow:hidden;flex:1;mask-image:linear-gradient(90deg,transparent,#000 4%,#000 96%,transparent)}
    .ticker-track{display:flex;width:max-content;gap:30px;padding:0 24px;animation:oddsTicker 30s linear infinite;white-space:nowrap}
    .ticker-track:hover{animation-play-state:paused}.ticker-item{font-size:12px;color:var(--muted)}.ticker-item strong{color:var(--text)}.ticker-edge{color:var(--green);font-weight:800;margin-left:7px}
    @keyframes oddsTicker{from{transform:translateX(0)}to{transform:translateX(-50%)}}
    .pulse-panel{display:grid;grid-template-columns:1.3fr repeat(4,1fr);gap:10px;margin:0 0 22px}
    .pulse-hero,.pulse-mini{border:1px solid var(--border);background:linear-gradient(145deg,rgba(52,211,153,.08),rgba(13,19,29,.94) 55%);border-radius:16px;padding:16px;position:relative;overflow:hidden}
    .pulse-hero:after{content:"";position:absolute;width:120px;height:120px;border-radius:50%;background:rgba(52,211,153,.08);filter:blur(25px);right:-35px;top:-55px}.pulse-kicker{font-size:9px;letter-spacing:.16em;text-transform:uppercase;color:var(--green);font-weight:900}.pulse-title{font-size:18px;font-weight:850;margin-top:5px}.pulse-sub{font-size:11px;color:var(--muted);margin-top:5px}.pulse-value{font-size:23px;font-weight:900;letter-spacing:-.03em;margin-top:5px}.pulse-label{font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:.09em;font-weight:750}
    .watch-btn{margin-left:auto;width:34px;height:34px;border-radius:10px;border:1px solid var(--border-light);background:#0a1018;color:#64748b;cursor:pointer;font-size:18px;display:grid;place-items:center;transition:.18s ease}.watch-btn:hover{transform:translateY(-1px);border-color:var(--green);color:var(--green)}.watch-btn.saved{color:#fbbf24;border-color:rgba(251,191,36,.45);background:rgba(251,191,36,.08);box-shadow:0 0 20px rgba(251,191,36,.08)}
    .watch-toast{position:fixed;right:20px;bottom:20px;z-index:9998;padding:12px 15px;border:1px solid var(--border-light);border-radius:12px;background:#0d131d;color:var(--text);box-shadow:0 18px 60px rgba(0,0,0,.4);font-size:12px;font-weight:700;opacity:0;transform:translateY(12px);pointer-events:none;transition:.2s}.watch-toast.show{opacity:1;transform:none}
    .kbd-hint{font-size:10px;color:var(--muted);margin-left:8px}.kbd-hint kbd{border:1px solid var(--border-light);background:#0a1018;border-radius:5px;padding:2px 6px;color:#cbd5e1}
    @media(max-width:900px){.pulse-panel{grid-template-columns:1fr 1fr}.pulse-hero{grid-column:1/-1}.ticker-label{display:none}}


    /* ======================================================
       ODDSCOPE NEXT LEVEL: command deck + edge leaderboard
       ====================================================== */
    .next-grid{display:grid;grid-template-columns:1.15fr .85fr;gap:12px;margin:0 0 22px}.edge-board,.quick-deck{border:1px solid var(--border);border-radius:16px;background:linear-gradient(155deg,rgba(96,165,250,.055),rgba(13,19,29,.96) 45%);padding:17px}.next-head{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:13px}.next-title{font-size:14px;font-weight:850}.next-sub{font-size:10px;color:var(--muted);margin-top:3px}.edge-row{display:grid;grid-template-columns:minmax(0,1.5fr) 80px 72px;align-items:center;gap:10px;padding:9px 0;border-top:1px solid rgba(41,53,72,.65)}.edge-row:first-of-type{border-top:0}.edge-name{font-size:11px;font-weight:750;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.edge-meta{font-size:9px;color:var(--muted);margin-top:2px}.edge-bar{height:6px;border-radius:999px;background:#172131;overflow:hidden}.edge-fill{height:100%;border-radius:inherit;background:linear-gradient(90deg,var(--blue),var(--green));box-shadow:0 0 12px rgba(52,211,153,.25)}.edge-number{text-align:right;color:var(--green);font-size:11px;font-weight:850}.deck-actions{display:grid;grid-template-columns:1fr 1fr;gap:8px}.deck-btn{border:1px solid var(--border-light);background:#0a1018;color:#cbd5e1;border-radius:11px;padding:12px;text-align:left;cursor:pointer;transition:.18s}.deck-btn:hover{border-color:var(--green);transform:translateY(-1px);color:#fff}.deck-icon{font-size:16px}.deck-name{font-size:10px;font-weight:800;margin-top:6px}.deck-key{font-size:9px;color:var(--muted);margin-top:2px}.command-backdrop{position:fixed;inset:0;z-index:10000;background:rgba(2,6,12,.72);backdrop-filter:blur(8px);display:none;align-items:flex-start;justify-content:center;padding-top:min(16vh,140px)}.command-backdrop.open{display:flex}.command-box{width:min(620px,calc(100vw - 28px));border:1px solid #334155;background:#0b111b;border-radius:18px;box-shadow:0 30px 100px rgba(0,0,0,.6);overflow:hidden}.command-search{width:100%;box-sizing:border-box;border:0;border-bottom:1px solid var(--border);background:transparent;color:#fff;padding:18px;font:600 14px inherit;outline:0}.command-list{padding:8px;max-height:390px;overflow:auto}.command-item{display:flex;align-items:center;justify-content:space-between;width:100%;border:0;background:transparent;color:#cbd5e1;padding:11px 12px;border-radius:10px;cursor:pointer;text-align:left}.command-item:hover,.command-item.active{background:var(--green-soft);color:#fff}.command-item small{color:var(--muted)}.command-footer{padding:9px 14px;border-top:1px solid var(--border);font-size:9px;color:var(--muted)}
    @media(max-width:800px){.next-grid{grid-template-columns:1fr}.edge-row{grid-template-columns:minmax(0,1fr) 65px 60px}}
...
@media(max-width:900px){.parlay-grid{grid-template-columns:1fr}.parlay-form{grid-template-columns:1fr 1fr}}@media(max-width:600px){.parlay-form{grid-template-columns:1fr}}
    ...
    document.addEventListener("DOMContentLoaded", installWatchButtons);
</script>
</head>
...
<div class="watch-toast" id="watch-toast">Saved to watchlist</div>
...
<div class="page">
    ...
    {% if signals %}
    <div class="signal-ticker" aria-label="Live signal ticker">
        <div class="ticker-label">MARKET TAPE</div>
        <div class="ticker-window"><div class="ticker-track">
            {% for pass in [1,2] %}{% for signal in signals[:8] %}
            <span class="ticker-item"><strong>{{ signal.away }} @ {{ signal.home }}</strong> · {{ signal.title }} · {{ signal.sportsbook or signal.away_book or '' }} <span class="ticker-edge">{{ signal.edge_display }}</span></span>
            {% endfor %}{% endfor %}
        </div></div>
    </div>
    {% endif %}

    <section class="pulse-panel" aria-label="OddsScope market pulse">
        <div class="pulse-hero"><div class="pulse-kicker">OddsScope Pulse</div><div class="pulse-title">Your live market command center</div><div class="pulse-sub">Scan the board, save games, and jump straight to the prices that matter.</div></div>
        <div class="pulse-mini"><div class="pulse-label">Top Edge</div><div class="pulse-value">{% if signals %}{{ signals[0].edge_display }}{% else %}—{% endif %}</div><div class="pulse-sub">Largest current discrepancy</div></div>
        <div class="pulse-mini"><div class="pulse-label">Strong Signals</div><div class="pulse-value">{{ signals|selectattr('strength','equalto','strong')|list|length }}</div><div class="pulse-sub">High-priority market flags</div></div>
        <div class="pulse-mini"><div class="pulse-label">Live Games</div><div class="pulse-value">{{ games|selectattr('live','equalto',true)|list|length }}</div><div class="pulse-sub">On the board right now</div></div>
        <div class="pulse-mini"><div class="pulse-label">Watchlist</div><div class="pulse-value" id="watch-count">0</div><div class="pulse-sub">Saved in this browser</div></div>
    </section>

    <section class="next-grid" aria-label="OddsScope edge leaderboard and quick actions">
        <div class="edge-board">
            <div class="next-head"><div><div class="pulse-kicker">Edge Leaderboard</div><div class="next-title">Strongest market discrepancies</div><div class="next-sub">Ranked from the live signal engine · market-derived, not predictions</div></div><span class="intel-live"><span class="intel-live-dot"></span> SCANNING</span></div>
            {% if signals %}{% set max_edge = signals[0].edge if signals[0].edge else 0.01 %}
            {% for signal in signals[:5] %}
            <div class="edge-row"><div><div class="edge-name">{{ signal.title }} · {{ signal.away }} @ {{ signal.home }}</div><div class="edge-meta">{{ signal.sportsbook or signal.away_book or 'Cross-book' }} · {{ signal.category }}</div></div><div class="edge-bar"><div class="edge-fill" style="width:{{ [100, ((signal.edge or 0) / max_edge * 100)|round]|min }}%"></div></div><div class="edge-number">{{ signal.edge_display }}</div></div>
            {% endfor %}{% else %}<div class="next-sub">No qualifying discrepancies right now.</div>{% endif %}
        </div>
        <div class="quick-deck">
            <div class="next-head"><div><div class="pulse-kicker">Quick Deck</div><div class="next-title">Navigate OddsScope like a terminal</div><div class="next-sub">Press <b>Ctrl/⌘ + K</b> anywhere for Command Center</div></div></div>
            <div class="deck-actions">
                <button class="deck-btn" type="button" onclick="openCommandPalette()"><div class="deck-icon">⌘</div><div class="deck-name">Command Center</div><div class="deck-key">Ctrl/⌘ K</div></button>
                <button class="deck-btn" type="button" onclick="jumpTo('parlay-lab')"><div class="deck-icon">⚡</div><div class="deck-name">Parlay Lab</div><div class="deck-key">Build + compare</div></button>
                <button class="deck-btn" type="button" onclick="jumpTo('movement-lab')"><div class="deck-icon">↗</div><div class="deck-name">Movement Lab</div><div class="deck-key">Historical lines</div></button>
                <button class="deck-btn" type="button" onclick="activateNamedFilter('watch')"><div class="deck-icon">★</div><div class="deck-name">My Watchlist</div><div class="deck-key">Saved games</div></button>
            </div>
        </div>
    </section>

    <section class="intelligence" id="signals-section">
    ...
    <div class="controls">
        ...
        <div class="filters">
            ...
            <button
                class="filter-button"
                data-filter="watch"
                onclick="setFilter('watch', this)"
            >
                ★ Watchlist <span class="kbd-hint"><kbd>W</kbd></span>
            </button>
            ...
            >
                ...
                <div class="game-header">
                    ...
                    <div>
                        ...
                        <div class="matchup team-matchup-row">
                            <span class="team-lockup">
                                {% if game.away_logo_url %}<img class="team-logo" src="{{ game.away_logo_url }}" alt="{{ game.away }} logo" loading="lazy" onerror="this.hidden=true;">{% endif %}
                                <span>{{ game.away }}</span>
                            </span>
                            ...
                            <span class="team-lockup">
                                {% if game.home_logo_url %}<img class="team-logo" src="{{ game.home_logo_url }}" alt="{{ game.home }} logo" loading="lazy" onerror="this.hidden=true;">{% endif %}
                                <span>{{ game.home }}</span>
                            </span>
                        </div>
                    ...
                    <table>
                        ...
                        {% for book in game.books %}
                            ...
                            <tr>
                                ...
                                <td class="book-name">
                                    <span class="book-brand">
                                        {% if book.logo_urls %}
                                            <img class="sportsbook-logo" src="{{ book.logo_urls[0] }}" data-logo-candidates='{{ book.logo_urls|tojson }}' data-logo-index="0" alt="{{ book.name }} logo" loading="lazy" onerror="tryNextSportsbookLogo(this);">
                                        {% endif %}
                                        <span>{{ book.name }}</span>
                                    </span>
                                </td>
...
<div class="command-backdrop" id="command-palette" role="dialog" aria-modal="true" aria-label="OddsScope Command Center">
  <div class="command-box">
    <input class="command-search" id="command-search" placeholder="Search commands…" autocomplete="off">
    <div class="command-list" id="command-list">
      <button class="command-item" data-command="search"><span>⌕ Search teams</span><small>/</small></button>
      <button class="command-item" data-command="watch"><span>★ Open watchlist</span><small>W</small></button>
      <button class="command-item" data-command="live"><span>● Show live games</span><small>LIVE</small></button>
      <button class="command-item" data-command="signals"><span>◉ Jump to market signals</span><small>INTEL</small></button>
      <button class="command-item" data-command="parlay"><span>⚡ Open Parlay Lab</span><small>LAB</small></button>
      <button class="command-item" data-command="movement"><span>↗ Open Market Movement</span><small>CHART</small></button>
      <button class="command-item" data-command="top"><span>↑ Back to top</span><small>HOME</small></button>
    </div>
    <div class="command-footer">↑ ↓ navigate · Enter select · Esc close</div>
  </div>
</div>
...
<script>
    ...
    // ======================================================
    // ODDSCOPE PRO UX: watchlist, shortcuts, scroll meter
    // ======================================================
    const WATCH_KEY = "oddsscope_watchlist_v1";
    let watchedGames = new Set(JSON.parse(localStorage.getItem(WATCH_KEY) || "[]"));

    function saveWatchlist(){localStorage.setItem(WATCH_KEY, JSON.stringify([...watchedGames]));updateWatchUI();}
    function showWatchToast(message){const toast=document.getElementById("watch-toast");if(!toast)return;toast.textContent=message;toast.classList.add("show");clearTimeout(window.__watchToast);window.__watchToast=setTimeout(()=>toast.classList.remove("show"),1600);}
    function toggleWatch(id){id=String(id);if(watchedGames.has(id)){watchedGames.delete(id);showWatchToast("Removed from watchlist");}else{watchedGames.add(id);showWatchToast("★ Saved to watchlist");}saveWatchlist();if(activeFilter==="watch")applyFilters();}
    function tryNextSportsbookLogo(img) {
        let candidates = [];
        try { candidates = JSON.parse(img.dataset.logoCandidates || "[]"); } catch (_) {}
    ...
    function updateWatchUI(){const count=document.getElementById("watch-count");if(count)count.textContent=watchedGames.size;document.querySelectorAll(".game-card").forEach(card=>{const btn=card.querySelector(".watch-btn");if(!btn)return;const saved=watchedGames.has(String(card.dataset.gameId));btn.classList.toggle("saved",saved);btn.textContent=saved?"★":"☆";btn.title=saved?"Remove from watchlist":"Save to watchlist";});}
    function installWatchButtons(){document.querySelectorAll(".game-card").forEach(card=>{if(card.querySelector(".watch-btn"))return;const header=card.querySelector(".game-header");if(!header)return;const btn=document.createElement("button");btn.type="button";btn.className="watch-btn";btn.setAttribute("aria-label","Toggle watchlist");btn.addEventListener("click",()=>toggleWatch(card.dataset.gameId));header.appendChild(btn);});updateWatchUI();}
    window.addEventListener("scroll",()=>{const el=document.getElementById("scroll-progress");if(!el)return;const max=document.documentElement.scrollHeight-innerHeight;el.style.width=(max>0?(scrollY/max)*100:0)+"%";},{passive:true});
    document.addEventListener("keydown",event=>{if(event.target.matches("input,textarea,select"))return;if(event.key==="/"){event.preventDefault();document.getElementById("searchInput")?.focus();}if(event.key.toLowerCase()==="w"){const btn=document.querySelector('[data-filter="watch"]');if(btn)setFilter("watch",btn);}});
    ...
    function activateNamedFilter(name){const btn=document.querySelector(`[data-filter="${name}"]`);if(btn)setFilter(name,btn);jumpTo("gamesContainer");}
    function openCommandPalette(){const p=document.getElementById("command-palette");if(!p)return;p.classList.add("open");const q=document.getElementById("command-search");q.value="";filterCommands();setTimeout(()=>q.focus(),20);}
    function closeCommandPalette(){document.getElementById("command-palette")?.classList.remove("open");}
    function runCommand(cmd){closeCommandPalette();if(cmd==="search"){document.getElementById("searchInput")?.focus();return;}if(cmd==="watch"||cmd==="live"){activateNamedFilter(cmd);return;}if(cmd==="signals"){jumpTo("signals-section");return;}if(cmd==="parlay"){jumpTo("parlay-lab");return;}if(cmd==="movement"){jumpTo("movement-lab");return;}if(cmd==="top"){window.scrollTo({top:0,behavior:"smooth"});}}
    function visibleCommands(){return [...document.querySelectorAll(".command-item")].filter(x=>x.style.display!=="none");}
    function filterCommands(){const q=(document.getElementById("command-search")?.value||"").toLowerCase();document.querySelectorAll(".command-item").forEach(x=>{x.style.display=x.innerText.toLowerCase().includes(q)?"":"none";x.classList.remove("active");});visibleCommands()[0]?.classList.add("active");}
    document.getElementById("command-search")?.addEventListener("input",filterCommands);
    document.querySelectorAll(".command-item").forEach(x=>x.addEventListener("click",()=>runCommand(x.dataset.command)));
    document.getElementById("command-palette")?.addEventListener("click",e=>{if(e.target.id==="command-palette")closeCommandPalette();});
    document.addEventListener("keydown",e=>{if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==="k"){e.preventDefault();openCommandPalette();return;}const p=document.getElementById("command-palette");if(!p?.classList.contains("open"))return;if(e.key==="Escape"){closeCommandPalette();return;}if(e.key==="Enter"){e.preventDefault();document.querySelector(".command-item.active")?.click();return;}if(e.key==="ArrowDown"||e.key==="ArrowUp"){e.preventDefault();const items=visibleCommands();if(!items.length)return;let i=items.findIndex(x=>x.classList.contains("active"));items.forEach(x=>x.classList.remove("active"));i=e.key==="ArrowDown"?(i+1)%items.length:(i-1+items.length)%items.length;items[i].classList.add("active");items[i].scrollIntoView({block:"nearest"});}});
    ...
    async function loadMovementChart() {
        ...
        try {
            ...
            const isCollecting = points.length === 1;
            message.style.display = isCollecting ? "grid" : "none";
            if (isCollecting) {
                message.textContent = "Collecting history — 1 snapshot recorded. The timeline will extend as new snapshots are saved.";
            }

            // With only one snapshot, duplicate it visually so Chart.js renders a
            // short horizontal current-market line instead of a single floating dot.
            // The duplicate is display-only; opening/current stats still use the
            // real snapshot array below.
            const chartPoints = points.length === 1
                ? [points[0], {...points[0], time: new Date(new Date(points[0].time).getTime() + 60000).toISOString()}]
                : points;
            const labels = chartPoints.map(p => movementTimeLabel(p.time));
            const values = chartPoints.map(p => p.value);
            const counts = chartPoints.map(p => p.books);
            ...
            document.getElementById("movement-current-sub").textContent = points.length === 1
                ? "1 snapshot recorded · collecting history"
                : `${points.length} snapshots in selected window`;
    ...
    function applyFilters() {
        ...
        cards.forEach(card => {
            ...
            if (
                ...
                matchesFilter =
                    watchedGames.has(String(card.dataset.gameId));
...
def api_history(event_id):
    ...
    try:
        ...
        rows = get_market_history(event_id, column, hours)
        ...
        for stamp in sorted(grouped):
            ...
            # Keep every recorded minute, even when the market value did not change.
            # This makes the movement chart a true timeline instead of collapsing a
            # flat market into a single lonely point.
